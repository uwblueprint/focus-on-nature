...
class CampService implements ICampService {
  ...
  async deleteCampSessionsByIds(campId: string, campSessionIds: Array<string>): Promise<void> {
    try {
      ...
      if (campSessions == null || campSessions.length !== campSessionIds.length) {
        throw new Error(
      ...
      let currentCampSessionsSet = new Set(camp.campSessions);

      campSessionIds.forEach((sessionId) => {
        if (!(sessionId in currentCampSessionsSet)) {
          throw new Error(
      ...
      try {
        ...
        let sessionsToDeleteSet = new Set(campSessionIds);
        ...
        if (!updatedCampSessionCampers) {
          throw new Error(
            `Failed to update ${camp} with deleted campers.`,
          );
        }
