...
class CamperService implements ICamperService {
  ...
  async moveCampersToCampSession(
    camperIds: string[],
    ...
    try {
      ...
      oldCampers.forEach((oldCamper) => {
        if (oldCamper.chargeId !== oldCampers[0].chargeId) {
          throw new Error(`All campers must have the same chargeId.`);
        }
      ...
      if (
        ...
        throw new Error(
          `Error: new camp session ${campSessionId} does not have enough capacity`,
        );
      }
      ...
      return updatedCampers.map((updatedCamper) => ({
        id: updatedCamper.id,
        campSession: updatedCamper.campSession
          ? updatedCamper.campSession.toString()
          : "",
