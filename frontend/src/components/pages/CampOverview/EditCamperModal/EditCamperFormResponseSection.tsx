...
const EditCamperFormResponseSection = ({
    ...
    const renderMCQ = (question: string, answer: string | undefined, options: string[], questionId: string, isRequired: boolean) => {    
      return (
            ...
            <Stack direction="column">
              {options.map(option => (
                <Radio 
                  ...
                  value={option}
                  onChange={() => {updateFormResponse(question, option)}}
                >
                  {option}
                </Radio>
              ))}
    ...
    const renderText = (question : string, answer: string, questionId: string, isRequired: boolean) => {
      return (
        <FormControl key={questionId} minW="100%" isRequired={isRequired}>
          ...
          <Textarea 
            ...
            textAlign="start"
            defaultValue={answer}
            onChange={(event) => {updateFormResponse(question, event.target.value)}}
          />
          <br/>
        </FormControl>
      )
    ...
    const renderMultiSelect = (question : string, answer: string | undefined, options : string[], questionId: string, isRequired: boolean) => {
